...
const SPOTIFY_UA = 'A/2.0.3 (Spotify Web API bridge)';
...
function getSpotifyOAuthConfig() {
  ...
  // A owns the OAuth flow, so always include the scopes required by
...
function spotifyErrorDetails(err) {
  ...
  if (reauthRequired || statusCode === 401 || code === 'SPOTIFY_REFRESH_TOKEN_MISSING') {
    ...
    message = 'Spotify 服务暂时不可用，A 已完成有限重试，请稍后再试。';
...
async function handleSpotifyCreatePlaylist(name, opts) {
  ...
  const created = await spotifyUserRequest('/me/playlists', 'POST', {}, {
    ...
    public: isPublic,
    collaborative: false,
    description: normalizeText(opts.description || 'Created with A'),
  }, { timeoutMs: 10000 });
  return {
    provider: 'spotify',
    loggedIn: true,
...
async function handleSpotifySongUrl(track) {
  ...
  return {
    ...
    restriction: {
      category: 'provider_limited',
      reason: 'spotify_metadata_only',
      message: 'Spotify 官方 Web API 不提供可交给 A 播放的音频直链，正在自动换源。',
      action: 'switch_source',
    },
  };
}
...
async function handleSpotifyLyric(id) {
  return {
    ...
    ytlrc: '',
    source: 'none',
      message: 'Spotify Web API 不提供歌词，A 会沿用跨平台歌词兜底。',
  };
}
