...
function resolveRcedit(projectDir) {
  ...
  if (!hit) throw new Error('No usable rcedit executable was found for A icon injection.');
  return hit;
}
...
module.exports = async function afterPack(context) {
  ...
  const appName = context.packager.appInfo.productFilename || 'A';
  ...
  if (!fs.existsSync(exePath)) throw new Error(`A executable was not found: ${exePath}`);
  if (!fs.existsSync(iconPath)) throw new Error(`A icon was not found: ${iconPath}`);

  const version = context.packager.appInfo.version;
  console.log(`  • injecting A resources  rcedit=${rceditPath}`);
