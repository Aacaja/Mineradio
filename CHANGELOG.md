...
> 本文件现在同时保留原项目的历史记录。下面的“当前开发版”是 A 在原开源项目基础上的个人魔改内容；后面的旧版本条目仅用于追溯历史，不代表 A 当前的正式发布版本。

## 当前开发版（A 个人魔改）

日期：2026-08-01

- 将产品、运行时界面、启动页、图标、安装器、快捷方式和构建产物名称统一为 A，保留源码中的旧内部标识以兼容现有构建和用户数据。
- 新增从旧 `AppData/Mineradio` 到 `AppData/A` 的非破坏性应用数据迁移。
- 新增稳定的首页“音乐库”入口，完整浏览 Navidrome 专辑和歌单，不再依赖鼠标悬停左侧歌单栏。
- 新增专辑目录、专辑详情、歌单详情和“播放全部”入口，支持点击歌曲直接播放。
- 修复 Navidrome 能读取歌单但歌曲无法播放的问题，统一远程播放代理、鉴权和异常处理。
- 修复 Navidrome 歌曲、专辑和歌单封面读取问题，并为封面失败提供占位显示。
- 为长歌单增加分页续载和播放队列补页，避免固定截断或一次性渲染全部歌曲。
- 新增/完善本地音乐库：递归扫描常见音频格式，读取标签、内嵌封面、目录和专辑信息，并自动生成本地歌单。
- 修复 FLAC 内嵌歌词读取和同步歌词解析；没有内嵌歌词时继续支持同目录 `.lrc` 文件。
- 保留首页每日推荐、最近播放、继续听、歌词舞台、粒子视觉、桌面模式和 3D 歌单架。
- 增加媒体库回归测试，覆盖 Navidrome 播放地址、封面代理、FLAC 歌词和歌单路由等关键链路。

> 当前开发机为 macOS，Windows 安装包由 GitHub Actions 的 `Windows Build` 工作流生成。当前 `package.json` 的 `2.0.3` 仅是构建基线，尚未创建新的 A 正式版本号。
